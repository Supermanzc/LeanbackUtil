...
public class HorizontalGridView extends BaseGridView {
    ...
    private int mEndFadeColor = Color.WHITE;
...
//                + mHighFadeShaderOffset + mHighFadeShaderLength : getWidth();
        ...
        int highEdge = getWidth() + mHighFadeShaderOffset;
        if (!getClipToPadding() || !getClipChildren()) {
            lowEdge -= getPaddingLeft();
            highEdge += getPaddingRight();
        }
        ...
        if (needsFadingLow && mLowFadeShaderLength > 0) {
            mLowFadeShader = new LinearGradient(lowEdge, 0, lowEdge + mLowFadeShaderLength, 0,
                    mEndFadeColor, Color.TRANSPARENT, Shader.TileMode.CLAMP);
            mTempPaint.setShader(mLowFadeShader);
            canvas.drawRect(lowEdge,
                    0,
                    lowEdge + mLowFadeShaderLength,
                    getHeight(), mTempPaint);
        }
        // 右边
        if (needsFadingHigh && mHighFadeShaderLength > 0) {
            mHighFadeShader = new LinearGradient(highEdge - mHighFadeShaderLength,
                    0,
                    highEdge,
                    0,
                    new int[]{Color.TRANSPARENT, mEndFadeColor}, null, LinearGradient.TileMode.CLAMP);
            mTempPaint.setShader(mHighFadeShader);
            canvas.drawRect(highEdge - mHighFadeShaderLength, 0, highEdge, getHeight(), mTempPaint);
        }
    }
