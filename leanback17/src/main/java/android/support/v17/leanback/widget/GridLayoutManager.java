...
public class GridLayoutManager extends RecyclerView.LayoutManager {
    ...
    boolean mIsMidScroll = false;

    /**
     * 设置无论是删除还是添加新的控件.
     * 在少于整体屏幕宽度的情况下，保持在中间的位置.
     * @param isMidScroll true 保持中间位置 false 反之
     */
    public void setMidScroll(boolean isMidScroll) {
        this.mIsMidScroll = isMidScroll;
    }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (mIsMidScroll) {
            int c = getChildCount();
            if (c >= getItemCount()) {
                int pSize = mBaseGridView.getMeasuredWidth() - mBaseGridView.getPaddingLeft() - mBaseGridView.getPaddingRight();
                int allViewSize = 0;
                for (int i = 0; i < c; i++) {
                    View child = getChildAt(i);
                    int size = getDecoratedMeasuredWidthWithMargin(child);
                    allViewSize += size;
                }
                if (allViewSize > 0 && allViewSize < pSize) {
                    int padding = (pSize - allViewSize) / 2;
                    int space = 0;
                    if (c > 1) {
                        View spaceView = getChildAt(c - 1);
                        space = getLeftDecorationWidth(spaceView) + getRightDecorationWidth(spaceView);
                    }
                    if (padding > 0) {
                        offsetChildrenHorizontal(padding - space);
                    }
                }
            }
        }
    ...
    private void scrollToView(View view, View childView, boolean smooth) {
        ...
        if (getScrollPosition(view, childView, sTwoInts)) {
            ...
            // 用于移动边框适配.
            sSaveTwoInts[0] = sTwoInts[0];
            sSaveTwoInts[1] = sTwoInts[1];
        } else {
            // 用于移动边框适配.
            sSaveTwoInts[0] = 0;
            sSaveTwoInts[1] = 1;
    ...
    int[] sSaveTwoInts = new int[2];

    public int[] getsSaveTwoInts() {
        return sSaveTwoInts;
    }
    ...
    private void scrollGrid(int scrollPrimary, int scrollSecondary, boolean smooth) {
        if (mInLayout) {
            ...
            if (smooth) {
                mBaseGridView.smoothScrollBy(scrollX, scrollY, mInterpolator);
            } else {
...
//        return focused != null ? focused : mBaseGridView;
    ...
    /**
    * BUG: RecyclerView 的 focusSearch 调用 onFocusSearchFailed 无效，导致焦点无法控制.
    */
    @Override
